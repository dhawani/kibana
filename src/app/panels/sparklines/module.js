...
function (angular, app, $, _, kbn, moment, timeSeries) {
  ...
  module.directive('sparklinesChart', function(dashboard, $filter) {
    return {
      restrict: 'A',
      scope: {
        series: '=',
      ...
      link: function(scope, elem) {
        ...
        elem.bind("plothover", function (event, pos, item) {
          if (item) {
            $tooltip
              .html(
                item.datapoint[1] + " @ " + $filter('datetz')(item.datapoint[0], dashboard.current.timezone,'yyyy-MM-dd HH:mm:ss')
              )
